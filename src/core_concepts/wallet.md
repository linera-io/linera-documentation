...
To chain the default chain for you wallet, user the `set-default` command:
...
Opening a chain for another `wallet` (let's call it `wallet2`) requires an extra two
steps.

First `wallet2` must create an unassigned keypair. The public part of that keypair
is then set to the `wallet` who is the chain creator.
...
The first line is the Chain Id of the newly created chain. The second line is the operation certificate.

Finally, to add the chain to `wallet2` for the given unassigned key we use the `wallet assign` command:
